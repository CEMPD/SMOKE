...
set nb = normbeis4
set tb = tmpbeis4
if ($WRF_VERSION == 3 && ! $?USE_WRF_WSAT) then
  setenv USE_WRF_WSAT N
  echo "due to WRF_VERSION = 3, setting USE_WRF_WSAT to N"
endif
if ($WRF_VERSION != 3 && ! $?USE_WRF_WSAT) then
  setenv USE_WRF_WSAT Y
  echo "due to WRF_VERSION = 4, setting USE_WRF_WSAT to Y"
endif
   ...
   if ( $RUN_CEMSCAN == 'Y' && $RUN_PART0 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
         ...
         if ( $debugmode == Y ) then
            if (-e $debug_exe/cemscan) then
  	       $debug_exe/cemscan
               if ( $status != 0 ) then
                  echo ERROR detected in Cemscan
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running Cemscan debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $SMK_BIN/cemscan -m "Running Cemscan -- $emf_period" -p $emf_period ## Log to EMF server
               ...
               if ( $timelog_yn == Y ) then
                  $timetracker N $TIMELOG $startdt cemscan inv
                  if ( $status != 0 ) then
   ...
   if ( $RUN_SMKINVEN == 'Y' && $RUN_PART1 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
         ...
         if ( $debugmode == Y ) then
            if (-e $debug_exe/smkinven) then
  	       $debug_exe/smkinven
               if ( $status != 0 ) then
                  echo ERROR detected in smkinven
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running smkinven debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $checklog -m "Checking log $LOGFILE" -p $emf_period ## log w/ EMF server
      ...
      # Remove temporary file, environment variable set by Smkinven at runtime
      #   to include process ID.
      if ( $?SMKINVEN_TMPFILE ) then
         if ( -e $SMKINVEN_TMPFILE ) then
            /bin/rm -rf $SMKINVEN_TMPFILE
         endif

      # Support older versions of Smkinven, that hardwired tmpfile name
      else
   ...
   if ( $RUN_RAWBIO == 'Y' && $RUN_PART1 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program for standard or summer
         ...
         if ( $debugmode == Y ) then
            if (-e $debug_exe/rawbio) then
  	       $debug_exe/rawbio
               if ( $status != 0 ) then
                  echo ERROR detected in rawbio
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running rawbio debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
if ( $?RUN_NORMBEIS4 ) then

   if ( $RUN_NORMBEIS4 == 'Y' && $RUN_PART1 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program for standard or summer
         ...
         if ( $debugmode == Y ) then
            if (-e $debug_exe/$nb) then
  	       $debug_exe/$nb
               if ( $status != 0 ) then
                  echo ERROR detected in $nb
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running $nb debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
   ...
   if ( $RUN_SPCMAT == 'Y' && $RUN_PART1 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
         ...
         if ( $debugmode == Y ) then
            if (-e $debug_exe/spcmat) then
  	       $debug_exe/spcmat
               if ( $status != 0 ) then
                  echo ERROR detected in spcmat
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running spcmat debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
   ...
   if ( $RUN_GRDMAT == 'Y' && $RUN_PART1 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
         ...
         if ( $debugmode == Y ) then
            if (-e $debug_exe/grdmat) then
  	       $debug_exe/grdmat
               if ( $status != 0 ) then
                  echo ERROR detected in grdmat
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running grdmat debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
if ( $?RUN_MBSETUP ) then
   if ( $RUN_MBSETUP == 'Y' && $RUN_PART1 == Y ) then
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/mbsetup) then
  	       $debug_exe/mbsetup
               if ( $status != 0 ) then
                  echo ERROR detected in mbsetup
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running mbsetup debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
   ...
   if ( $RUN_PREMOBL == 'Y' && $RUN_PART1 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
         ...
         if ( $debugmode == Y ) then
            if (-e $debug_exe/premobl) then
  	       $debug_exe/premobl
               if ( $status != 0 ) then
                  echo ERROR detected in premobl
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running premobl debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
   ...
   if ( $RUN_METSCAN == 'Y' && $RUN_PART1 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
        setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/metscan) then
  	       $debug_exe/metscan
               if ( $status != 0 ) then
                  echo ERROR detected in metscan
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running metscan debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
if ( $?RUN_TEMPORAL ) then
   if ( $RUN_TEMPORAL == 'Y' && $RUN_PART2 == Y ) then
	 ...
	 source $movelog
      ...
      # C. Allen, 8 Aug 2014: This concerns MOBILE6 processing from years ago and is now obsolete
#      if ( $SMK_SOURCE == M ) then
...
#      endif
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/temporal) then
  	       $debug_exe/temporal
               if ( $status != 0 ) then
                  echo ERROR detected in temporal
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running temporal debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
set debugexestat = 0
set exestat = 0
setenv TMPLOG   $OUTLOG/elevpoint_${SRCABBR}_${CASE}_${GRID}.log
## (C. Allen) If ELEVPOINT_DAILY = Y, then Elevpoint is run for each day;
#  each day should have its own log
if ( $?ELEVPOINT_DAILY ) then
   if ( $ELEVPOINT_DAILY == Y ) then
      setenv TMPLOG   $OUTLOG/elevpoint_${SRCABBR}_${CASE}_${ESDATE}_${GRID}.log
   endif  
endif
if ( $?RUN_ELEVPOINT ) then
   if ( $RUN_ELEVPOINT == Y && $RUN_PART3 == Y ) then      
	 ...
	 source $movelog
      ...
      if ( -e $PTMP ) then
         setenv PTMPLIST $INTERMED/ptmplist_$$.txt
         if ( -e $PTMPLIST ) /bin/rm -rf $PTMPLIST

         ls $INTERMED/ptmp_${SRCABBR}*ncf > $PTMPLIST
      endif

      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/elevpoint) then
  	       $debug_exe/elevpoint
               if ( $status != 0 ) then
                  echo ERROR detected in elevpoint
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running elevpoint debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
   ...
   if ( $RUN_LAYPOINT == 'Y' && $RUN_PART4 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/laypoint) then
  	       $debug_exe/laypoint
               if ( $status != 0 ) then
                  echo ERROR detected in laypoint
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running laypoint debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
   ...
   if ( $RUN_TMPBIO == 'Y' && $RUN_PART2 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
        setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/tmpbio) then
  	       $debug_exe/tmpbio
               if ( $status != 0 ) then
                  echo ERROR detected in tmpbio
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running tmpbio debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
if ( $?RUN_TMPBEIS4 ) then
   if ( $RUN_TMPBEIS4 == 'Y' && $RUN_PART2 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
        setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/$tb) then
  	       $debug_exe/$tb
               if ( $status != 0 ) then
                  echo ERROR detected in $tb
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running $tb debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
if ( $?RUN_SMKMERGE ) then
   if ( $RUN_SMKMERGE == 'Y' && $RUN_PART4 == Y ) then
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/smkmerge) then
  	       $debug_exe/smkmerge
               if ( $status != 0 ) then
                  echo ERROR detected in smkmerge
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running smkmerge debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
set debugexestat = 0
set exestat = 0
setenv TMPLOG   $OUTLOG/movesmrg_${SRCABBR}_${CASE}_${ESDATE}_${GRID}_$SPC.log
if ( $?RUN_MOVESMRG ) then
   if ( $RUN_MOVESMRG == 'Y' && $RUN_PART4 == Y ) then
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/movesmrg) then
  	       $debug_exe/movesmrg
               if ( $status != 0 ) then
                  echo ERROR detected in movesmrg
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running movesmrg debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
	       echo 'SCRIPT ERROR: movesmrg program does not exist in:'
            ...
            if ( -e $SMK_BIN/movesmrg ) then
               ...
               set startdt = `date +%m/%d/%Y,%T`
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $SMK_BIN/movesmrg -m "Running movesmrg -- $emf_period" -p $emf_period ## Log to EMF server
               time $SMK_BIN/movesmrg
               if ( $timelog_yn == Y ) then
                  $timetracker N $TIMELOG $startdt movesmrg $runperiod
                  if ( $status != 0 ) then
                      echo "ERROR: Problem calling timetracker from smk_run script"
                      $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: Problem calling timetracker from smk_run script" -x $timetracker  -t "e" -p $emf_period ## log w/ EMF server
                      exit ( 1 )
...
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $checklog -m "Checking log $LOGFILE" -p $emf_period ## log w/ EMF server
               ...
               if ( $status != 0 ) then
                 echo "ERROR: detected in movesmrg"
		 $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: detected in movesmrg" -p $emf_period -t "e" ## Log to EMF Server
                 exit( 1 )
               endif
            else
               set exestat = 1 
	 ...
	 echo '              '$MG_SRC
      ...
      if ( $exitstat == 1 ) then
	 $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running movesmrg" -p $emf_period -t "e" ## Log to EMF Server
	 exit ( 1 )
      endif
   endif
endif
...
set exestat = 0
if ($?BEISLABEL) then
  setenv TMPLOG   $OUTLOG/mrggrid_${SRCABBR}_${CASE}_${ESDATE}_${GRID}_${SPC}_${BEISLABEL}.log
else
...
if ( $?RUN_MRGGRID ) then
   if ( $RUN_MRGGRID == 'Y' && $RUN_PART4 == Y ) then
	 ...
	 source $movelog
      ...
      # Set executable name depending on availablity of the extended Mrggrid
      #   (compiled with the I/O API library with MXVARS3 = 50000 instead of 120)
      set mrggrid_exec = mrggrid       # default to regular mrggrid
      if ( $?USE_MRGGRID_EXT ) then
         if ( $USE_MRGGRID_EXT == Y || $USE_MRGGRID_EXT == y ) then
            set mrggrid_exec = mrggrid_ext
         endif
      endif
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/mrggrid) then
  	       $debug_exe/mrggrid
               if ( $status != 0 ) then
                  echo ERROR detected in mrggrid
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running mrggrid debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
	       echo 'SCRIPT ERROR: mrggrid program does not exist in:'
            ...
            if ( -e $SMK_BIN/$mrggrid_exec ) then

               set startdt = `date +%m/%d/%Y,%T`
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $SMK_BIN/$mrggrid_exec -m "Running $mrggrid_exec -- $emf_period" -p $emf_period ## Log to EMF server
               time $SMK_BIN/$mrggrid_exec
               if ( $timelog_yn == Y ) then
                  $timetracker N $TIMELOG $startdt $mrggrid_exec $runperiod
                  if ( $status != 0 ) then
                      echo "ERROR: Problem calling timetracker from smk_run script"
                      $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: Problem calling timetracker from smk_run script" -x $timetracker  -t "e" -p $emf_period ## log w/ EMF server
                      exit ( 1 )
...
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $checklog -m "Checking log $LOGFILE" -p $emf_period ## log w/ EMF server
               $checklog
               if ( $status != 0 ) then
                 echo "ERROR: detected in $mrggrid_exec"
		 $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: detected in $mrggrid_exec" -p $emf_period -t "e" ## Log to EMF Server
                 exit( 1 )
               endif
            else
               set exestat = 1 
      ...
      if ( $exestat == 1 ) then
	 echo "SCRIPT ERROR: $mrggrid_exec program does not exist in:"
	 echo '              '$SMK_BIN
         set exitstat = 1
      endif

      if ( $debugexestat == 1 ) then
	 echo "SCRIPT ERROR: $mrggrid_exec.debug program does not exist in:"
	 echo '              '$MG_SRC
         set exitstat = 1
      endif
      if ( $exitstat == 1 ) then
	 $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running $mrggrid_exec" -p $emf_period -t "e" ## Log to EMF Server
	 exit ( 1 )
      endif
   endif
endif
   ...
   if ( $RUN_SMK2EMIS == 'Y' && $RUN_PART4 == Y ) then
	 ...
	 source $movelog
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/smk2emis) then
  	       $debug_exe/smk2emis
               if ( $status != 0 ) then
                  echo ERROR detected in smk2emis
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running smk2emis debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
...
set debugexestat = 0
set exestat = 0
setenv TMPLOG   $OUTLOG/layalloc_${SRCABBR}_${CASE}_${ESDATE}_${GRID}_$SPC.log
if ( $?RUN_LAYALLOC ) then
   if ( $RUN_LAYALLOC == 'Y' && $RUN_PART4 == Y ) then

      # Set input and output files
      setenv INPUT $EOUT.2D
      setenv OUTPUT $EOUT
      # Newer SMOKE versions use INFILE/OUTFILE instead of INPUT/OUTPUT
      setenv INFILE $INPUT
      setenv OUTFILE $OUTPUT 
      ...
      if ( $exitstat == 0 ) then         # Run program
         setenv LOGFILE $TMPLOG
         if ( $debugmode == Y ) then
            if (-e $debug_exe/layalloc) then
  	       $debug_exe/layalloc
               if ( $status != 0 ) then
                  echo ERROR detected in layalloc
                  $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running layalloc debug version" -p $emf_period -t "e" ## Log to EMF Server
                  exit( 1 )
               endif
	    else
	       echo 'SCRIPT ERROR: layalloc program does not exist in:'
  	       echo '              '$debug_exe
               set exitstat = 1
	    endif
         else
            if ( -e $SMK_BIN/layalloc ) then

               set startdt = `date +%m/%d/%Y,%T`
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $SMK_BIN/layalloc -m "Running layalloc -- $emf_period" -p $emf_period ## Log to EMF server
               time $SMK_BIN/layalloc
               if ( $timelog_yn == Y ) then
                  $timetracker N $TIMELOG $startdt layalloc $runperiod
                  if ( $status != 0 ) then
                      echo "ERROR: Problem calling timetracker from smk_run script"
                      $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: Problem calling timetracker from smk_run script" -x $timetracker  -t "e" -p $emf_period ## log w/ EMF server
                      exit ( 1 )
...
#	       $EMF_CLIENT -k $EMF_JOBKEY -x $checklog -m "Checking log $LOGFILE" -p $emf_period ## log w/ EMF server
               ...
               if ( $status != 0 ) then
                 echo "ERROR: detected in layalloc"
		 $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: detected in layalloc" -p $emf_period -t "e" ## Log to EMF Server
                 exit( 1 )
               endif
            else
               set exestat = 1 
	 ...
	 echo '              '$SMK_BIN
      ...
      if ( $debugexestat == 1 ) then
	 echo 'SCRIPT ERROR: layalloc.debug program does not exist in:'
	 echo '              '$MG_SRC
         set exitstat = 1
      endif
      if ( $exitstat == 1 ) then
	 $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running layalloc" -p $emf_period -t "e" ## Log to EMF Server
	 exit ( 1 )
      endif
   endif
endif
