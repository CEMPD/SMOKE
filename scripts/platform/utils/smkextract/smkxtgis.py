...
def load_config(config_path):
	...
	Returns: dict with resolved paths for inputs/outputs/county_shp/input_shapefile and ff10 settings.
	"""
	if not os.path.isfile(config_path):
		raise FileNotFoundError(f"Configuration file not found: {config_path}")
	with open(config_path, 'r') as cfg_file:
		config = json.load(cfg_file)
	required_keys = ['inputs', 'outputs', 'sector', 'county_shp', 'input_shp']
	...
	input_shapefile_path = config['input_shp']

	for key, value in [('inputs', inputs_dir), ('outputs', outputs_dir), ('county_shp', county_shp_path), ('input_shp', input_shapefile_path)]:
	...
	if not os.path.isabs(input_shapefile_path):
		input_shapefile_path = os.path.normpath(os.path.join(config_dir, input_shapefile_path))
	return {
		'inputs': inputs_dir,
		'outputs': outputs_dir,
		...
		'input_shp': input_shapefile_path,
	}

if __name__ == "__main__":
	default_config_name = "smkxtgis.json"
	parser = argparse.ArgumentParser(description="Filter FF10 inventories to counties intersecting a target shapefile.")
	parser.add_argument("--config", default=None, help="Path to JSON configuration with required inputs/outputs/sector/shapefile settings.")
	...
	input_shapefile = config['input_shp']
	...
	print(f"Input shapefile resolved to {input_shapefile}", flush=True)
	os.makedirs(outputs, exist_ok=True)

	print(f"Reading input shapefile {input_shapefile}...\n", flush=True)
	domain = gpd.read_file(input_shapefile)
	...
	for sector,fnames in sector_mapping.items():
		...
		for fname in fnames:
			...
			shapefile_name = os.path.splitext(os.path.basename(input_shapefile))[0]
			output_ff10 = os.path.join(sector_output, f"{ff10_base}_{shapefile_name}{ff10_ext}")
