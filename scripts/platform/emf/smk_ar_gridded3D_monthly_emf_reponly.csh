...
# using gridded EDGAR inventories from HTAP, for sectors with layering.
#
...
# Based on the script smk_ar_monthly_emf.csh
#
#*********************************************************************
...
set run_layalloc = Y       #  run layer allocation program

if ($?GCNTL) then  # if GCNTL exists, run Cntlmat; otherwise don't run Cntlmat
   set run_cntlmat = Y        #  run control matrix program
...
setenv RUN_CNTLMAT   $run_cntlmat
...
foreach m ( $MONTHS_LIST )   # MONTHS_LIST set by set_months.csh
   ...
   #    are EMF datasets. But for monthly sectors, the EMF dataset lists all months,
   #    and for Smkinven we only want a .lst file with one month. So, we will autogenerate a .lst file
   #    from the EMF dataset EMISLST by extracting the current month.
   
   grep "^#" $EMISLST > $LISTFILE
   grep ",$m," $EMISLST >> $LISTFILE
   ...
   if ($?INSTALL_DIR) then
      ...
      sed "s/$search_string/$replace_string/g" $LISTFILE > ${LISTFILE}_tmp
      mv -f ${LISTFILE}_tmp $LISTFILE
   endif
   ...
   ## Set period for Smkinven so that the correct HTAP inventory month is used
...
#                if ( -e $REPORT22B ) $EMF_CLIENT -k $EMF_JOBKEY -F $REPORT22B -T "Smkreport state-SCC-spec_profile" -O "state/SCC/EVP__VOC profile Smkreport $SUBSECT"
	     ...
	     if ( $REGISTER_REPOUT == Y ) then
#                echo "SCRIPT NOTE: Registering internal Smkreport reports"
#               if ( -e $REPORT22C ) $EMF_CLIENT -k $EMF_JOBKEY -F $REPORT22C -T "Smkreport state-SCC-spec_profile" -O "state/SCC/RFL__VOC profile Smkreport $SUBSECT"
	     endif # register_repout
          endif # end if qa script failure

      endif # end if sector not afdust/ag
...
#      endif
   ...
   while ( $n < $ndays )
      ...
      echo "smk_run_dates_1 = $SMK_RUN_DATES_1"
	 ...
	 if ( $L_TYPE == aveday ) then # For aveday, every day is Tuesday
	    ...
	    set wed_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Tuesday  | grep " $YEAR$mon2" | cut -c1-7`
	    set thu_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Tuesday  | grep " $YEAR$mon2" | cut -c1-7`
	    set fri_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Tuesday  | grep " $YEAR$mon2" | cut -c1-7`
	 ...
	 else if ( $L_TYPE == week ) then
	    set mon_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Monday  | grep " $YEAR$mon2" | cut -c1-7`
	    set tue_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Tuesday  | grep " $YEAR$mon2" | cut -c1-7`
	    set wed_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Wednesday  | grep " $YEAR$mon2" | cut -c1-7`
	    set thu_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Thursday  | grep " $YEAR$mon2" | cut -c1-7`
	    set fri_date = `grep -v " H " $SMK_RUN_DATES_1 | grep Friday  | grep " $YEAR$mon2" | cut -c1-7`
	 ...
	 echo "weddate is $wed_date"
	 echo "thudate is $thu_date"
	 echo "fridate is $fri_date"
	 echo "satdate is $sat_date"
	 echo "sundate is $sun_date"

	 # Set PTMP files for BYDAY approach
	 setenv ATMP_MON  $INTERMED/atmp_${SUBSECT}_${CASE}_$mon_date.ncf
	 setenv ATMP_TUE  $INTERMED/atmp_${SUBSECT}_${CASE}_$tue_date.ncf
	 setenv ATMP_WED  $INTERMED/atmp_${SUBSECT}_${CASE}_$wed_date.ncf
	 setenv ATMP_THU  $INTERMED/atmp_${SUBSECT}_${CASE}_$thu_date.ncf
	 setenv ATMP_FRI  $INTERMED/atmp_${SUBSECT}_${CASE}_$fri_date.ncf
	 setenv ATMP_SAT  $INTERMED/atmp_${SUBSECT}_${CASE}_$sat_date.ncf
	 setenv ATMP_SUN  $INTERMED/atmp_${SUBSECT}_${CASE}_$sun_date.ncf
	 ...
	 goto end_of_script
      ...
      # Zip the smkmerge and layalloc output if ZIPOUT = Y
      if ( $?ZIPOUT ) then
         if ( $ZIPOUT == Y ) then
	    gzip -vf $EOUT
	    gzip -vf $EOUT.2D
