...
if ( $?MOVES_TYPE ) then
  if ( $MOVES_TYPE == "RPD" || $MOVES_TYPE == "RPH" || $MOVES_TYPE == "RPHO" || $MOVES_TYPE == "RPS" ) then
    set run_temporal = Y       #  run temporal allocation program
  else
    set run_temporal = N       #  run temporal allocation program
  endif
else
  echo "SCRIPT ERROR: Environment variable MOVES_TYPE undefined. Please set to RPD, RPP, RPV, RPH, RPHO, or RPS."
...
switch ( $MOVES_TYPE )
  ...
  case "RPH":
    setenv RPH_MODE "Y"
    breaksw
  case "RPHO": # C.Allen update 13dec2019
    setenv ONI_MODE "Y"
    breaksw
  case "RPS": # C.Allen update 13may2020
    setenv RPS_MODE "Y"
    breaksw
  default
    echo "SCRIPT ERROR: Environment variable MOVES_TYPE must be set to RPD, RPP, RPV, RPH, RPHO, or RPS."
    $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: Environment variable MOVES_TYPE not set to RPD, RPP, RPV, RPH, RPHO, or RPS" -t "e"
    exit( 1 )
endsw
...
if (! $?GSREF && ! $?GSREF_SUMMER && ! $?GSREF_WINTER) then # Create the GSREF file from the pieces, if full GSREF and seasonal GSREFs not already defined
...
#  Only do this if GSREF is already defined (it is not yet at this point if using seasonal GSREF)
if ($?GSREF) then
...
if ($?MTREF) then 
  setenv MTREF_ANNUAL $MTREF
else
  setenv MTREF_ANNUAL "" # this will cause a SMOKE error
endif

if ($?SPDIST) then 
  setenv SPDIST_ANNUAL $SPDIST
else
  setenv SPDIST_ANNUAL "" # don't necessarily need an SPDIST
endif
...
foreach m ( $MONTHS_LIST )   # MONTHS_LIST set by set_months.csh
   ...
   # Will we use a month-specific GSREF for starts?
   set gsref_starts_month = 0
   env | grep "^GSREF_STARTS_${m}=" > /dev/null
   if ($status == 0 && $MOVES_TYPE == RPS) set gsref_starts_month = 1
   
   # use seasonal GSREF if defined
   if (($?GSREF_SUMMER || $?GSREF_WINTER) && $gsref_starts_month == 0) then
      echo "Attempting to use seasonal GSREF because either GSREF_SUMMER or GSREF_WINTER is defined"
      if ($m >= 5 && $m <= 9) then
         if ($?GSREF_SUMMER) then
            setenv GSREF $GSREF_SUMMER
	    echo "Month $m, using $GSREF_SUMMER"
	 else
            echo "ERROR: GSREF_WINTER is defined but GSREF_SUMMER is not defined for summer month"
            $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: GSREF_WINTER is defined but GSREF_SUMMER is not defined for summer month" -t "e"
            exit ( 1 )
	 endif
      else
         if ($?GSREF_WINTER) then
            setenv GSREF $GSREF_WINTER
	    echo "Month $m, using $GSREF_WINTER"
	 else
            echo "ERROR: GSREF_SUMMER is defined but GSREF_WINTER is not defined for winter month"
            $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: GSREF_SUMMER is defined but GSREF_WINTER is not defined for winter month" -t "e"
            exit ( 1 )
	 endif
      endif	 
   endif
   if ($gsref_starts_month) then
      set gsref_x = `env | grep "^GSREF_STARTS_${m}=" | cut -f2 -d'='`
      setenv GSREF $gsref_x
      echo "For RPS / starts, using month-specific GSREF $GSREF"
   endif

   # use monthly MTREF if it exists
   env | grep "^MTREF_${m}=" > /dev/null
   if ($status == 0) then
      set mtref_x = `env | grep "^MTREF_${m}=" | cut -f2 -d'='`
      setenv MTREF $mtref_x
      echo "Using month-specific MTREF $MTREF"
   else
      setenv MTREF $MTREF_ANNUAL
   endif
   
   # use monthly SPDIST if it exists
   env | grep "^SPDIST_${m}=" > /dev/null
   if ($status == 0) then
      set spdist_x = `env | grep "^SPDIST_${m}=" | cut -f2 -d'='`
      setenv SPDIST $spdist_x
      echo "Using month-specific SPDIST $SPDIST"
   else
      setenv SPDIST $SPDIST_ANNUAL
   endif
   
   if ($m == 0) then
      setenv SUBSECT ${MOVES_TYPE}_${SECTOR}_annual # set variable for input/output names
   ...
   #  This = 0 for RPP and RPV, and the month # for RPD, RPH, RPHO, and RPS.
   setenv SMKINVEN_MONTH 0
   if ($MOVES_TYPE == RPD) setenv SMKINVEN_MONTH $m
   if ($MOVES_TYPE == RPH) setenv SMKINVEN_MONTH $m
   if ($MOVES_TYPE == RPHO) setenv SMKINVEN_MONTH $m
   if ($MOVES_TYPE == RPS) setenv SMKINVEN_MONTH $m
   ...
   # Remove warnings from Spcmat log which are formatted badly and cause log analyzer to crash (Christine Allen, 4 May 2023)
   if (-e $LOGS/spcmat_${SRCABBR}_${CASE}_$SPC.log) then
      grep -va 'WARNING: XREF fracs do not add up to 1 for' $LOGS/spcmat_${SRCABBR}_${CASE}_$SPC.log > $LOGS/spcmat_${SRCABBR}_${CASE}_$SPC.log.tmp
      if ($status == 0) echo "Removing 'XREF fracs do not add up to 1' warnings from Spcmat log due to bad formatting in log"
      mv -f $LOGS/spcmat_${SRCABBR}_${CASE}_$SPC.log.tmp $LOGS/spcmat_${SRCABBR}_${CASE}_$SPC.log
   endif
...
#                                -O "grid cell w/county Smkreport $SECTOR (External)"
   ...
   if ($MOVES_TYPE == "RPP" || $MOVES_TYPE == "RPV") then 
   	setenv RUN_TEMPORAL N # in case run settings turns temporal on; we always want Temporal off for RPP and RPV
   endif
...
#      endif
      ...
      # New support for subsector outputs, added 25feb2021      
      if (! $?SMK_SUB_SECTOR_OUTPUT_YN ) then
         setenv SMK_SUB_SECTOR_OUTPUT_YN N
      endif
	...
	if ($days_this_run != 1) then
	  ...
	  if ($SMK_SUB_SECTOR_OUTPUT_YN == Y) then # supports up to 10 subsectors for now
	       setenv SUBOUT01 $PREMERGED/../../$SECTOR01/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR01}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT02 $PREMERGED/../../$SECTOR02/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR02}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT03 $PREMERGED/../../$SECTOR03/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR03}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT04 $PREMERGED/../../$SECTOR04/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR04}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT05 $PREMERGED/../../$SECTOR05/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR05}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT06 $PREMERGED/../../$SECTOR06/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR06}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT07 $PREMERGED/../../$SECTOR07/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR07}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT08 $PREMERGED/../../$SECTOR08/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR08}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT09 $PREMERGED/../../$SECTOR09/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR09}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	       setenv SUBOUT10 $PREMERGED/../../$SECTOR10/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${SECTOR10}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
	  endif
	endif
      ...
      if ($?days_this_run) then
	    ...
	    while ($j < $days_this_run)
		 ...
		 setenv OUTFILE $OUTPUT/$SECTOR/$MOVES_TYPE/sginln_${MM}_${MOVES_TYPE}_${SECTOR}_${thisday_greg}_${GRID}_${SPC}_${CASE}.ncf
	       ...
	       # Do m3xtract on all subsector files, if they exist 
               if ($SMK_SUB_SECTOR_OUTPUT_YN == Y) then # supports up to 10 subsectors for now	       
	       
	          foreach sectorx ( $SECTOR01 $SECTOR02 $SECTOR03 $SECTOR04 $SECTOR05 $SECTOR06 $SECTOR07 $SECTOR08 $SECTOR09 $SECTOR10 )
	       
	             setenv INFILE $PREMERGED/../../$sectorx/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${sectorx}_${ESDATE}_${GRID}_${SPC}_${CASE}_multiday.ncf
		     # if this is the second attempt at m3xtract, INFILE may have been previously zipped. check for that
		     if (! -e $INFILE && -e $INFILE.gz) then # if this  is 
		        gunzip $INFILE.gz
		     endif
		     if (! -e $INFILE) continue # only want to run m3xtracts on subout files that actually exist
		  
	             setenv OUTFILE $PREMERGED/../../$sectorx/$MOVES_TYPE/emis_${MM}_${MOVES_TYPE}_${sectorx}_${thisday_greg}_${GRID}_${SPC}_${CASE}.ncf
	             setenv LOGFILE $INTERMED/logs/m3xtract_${MOVES_TYPE}_${sectorx}_${thisday_greg}_${GRID}_${SPC}_$CASE.log
	             if (-e $OUTFILE) rm -f $OUTFILE
	             if (-e $LOGFILE) rm -f $LOGFILE
	             ...
	             echo "Multi-day run note: Creating single-day file via m3xtract for $thisday_greg for subsector $sectorx"
	             ...
	             $IOAPIDIR/m3xtract < $m3xtract_in
                     if ( $status != 0 ) then
      	                echo "ERROR: running m3xtract for $thisday_greg"
	                $EMF_CLIENT -k $EMF_JOBKEY -m "ERROR: running m3xtract for $thisday_greg, subsector $sectorx" -t "e" -x m3xtract -p $EMF_PERIOD
                        set exitstat = 1
                        goto end_of_script
                     endif
	    ...
	    if ($SMK_SUB_SECTOR_OUTPUT_YN == Y) then
	      if (-e $SUBOUT01) gzip -vf $SUBOUT01
	      if (-e $SUBOUT02) gzip -vf $SUBOUT02
	      if (-e $SUBOUT03) gzip -vf $SUBOUT03
	      if (-e $SUBOUT04) gzip -vf $SUBOUT04
	      if (-e $SUBOUT05) gzip -vf $SUBOUT05
	      if (-e $SUBOUT06) gzip -vf $SUBOUT06
	      if (-e $SUBOUT07) gzip -vf $SUBOUT07
	      if (-e $SUBOUT08) gzip -vf $SUBOUT08
	      if (-e $SUBOUT09) gzip -vf $SUBOUT09
	      if (-e $SUBOUT10) gzip -vf $SUBOUT10
	    endif
