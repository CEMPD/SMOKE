...
def check_fug_params(aer, inv):
    ...
    cols = ['facility_id','src_id','fug_height','fug_width_xdim','fug_length_ydim','fug_angle']
    # Convert meters to feet
    aer['fug_height'] = aer['rel_ht'] * 3.2808
    aer['fug_width_xdim'] = aer['x_length'] * 3.2808
    aer['fug_length_ydim'] = aer['y_length'] * 3.2808
    ...
    print('\nFug len/wid <= 0:')
    print(aer[(aer['fug_width_xdim'] <= 0) | (aer['fug_length_ydim'] <= 0)])
    return df
...
def loc_check(sector, work_path):
    ...
    print('\nDuplicate sources in locations file:')
    ...
    if sector.startswith('ptegu') or 'combined' in sector:
        ...
        if hr_list:
            for fn in hr_list:
                ...
                temp.to_csv(os.path.join(work_path, 'qa', 'csvs', 'temporal_hour_%s_check.csv' %sector), index=False)
            hr = temp[['facility_id','src_id']].copy()
        else:
            print('No hourly temporal files found')
    return hr
...
def check_temporal(sector, work_path):
    ...
    if 'ptegu' not in sector:
        ...
        temp.to_csv(os.path.join(work_path, 'qa', 'csvs', '%s_temporal_check.csv' %sector), index=False, 
...
def check_emis(sector, work_path):
    ...
    fn = os.path.join(work_path, 'emis', '%s_srcid_emis.csv' %sector)
    dtype = {'facility_id': str, 'ann_value': float}
    emis = pd.read_csv(fn, dtype=dtype, usecols=['facility_id','src_id','smoke_name','ann_value']) 
    emis = emis.groupby(['facility_id','src_id','smoke_name'], as_index=False).sum()
    return emis

def check_pr_emis(sector, work_path):
    # Read in the point emissions helper file
    fn = os.path.join(work_path, 'xwalk', '%s_process_releasept_emis.csv' %sector)
    dtype = {'facility_id': str, 'ann_value': float}
    emis = pd.read_csv(fn, dtype=dtype, usecols=['facility_id','src_id','smoke_name','ann_value']) 
    emis = emis.groupby(['facility_id','src_id','smoke_name'], as_index=False).sum()
    return emis
...
def get_inv(sector, inv_list, invtable):
    ...
    usecols = ['facility_id','poll','ann_value','fac_source_type','rel_point_id','unit_id',
      ...
      'fug_angle','fug_width_xdim','fug_length_ydim','oris_facility_code','oris_boiler_id']
    ...
    idx = ['facility_id','unit_id','rel_point_id','process_id','oris_facility_code','oris_boiler_id','keep']
    if len(inv[inv.duplicated(idx)]) > 0:
        print('Duplicate inventory emissions source:')
        print(inv[inv.duplicated(idx)].head())
    return inv
...
def main():
    ...
    if not os.path.exists(os.path.join(work_path, 'qa')):
        ...
        os.makedirs(os.path.join(work_path, 'qa', 'csvs'))
    qa_counts = []
    ...
    temp[(temp['s'].isnull()) | (temp['z'].isnull())].to_csv(os.path.join(work_path, 'qa', 'csvs',
      'missing_%s_sources.csv' %sector), index=False)
    # Get the emis information
    emis = check_pr_emis(sector, work_path)
    ...
    if len(inv[inv['smoke_name'].isnull()]) > 0:
        print('Pollutants in FF10 not in invtable:')
        print(list(inv.loc[inv['smoke_name'].isnull(), 'poll'].drop_duplicates()))
    ...
    invtable = invtable[['smoke_name','keep']].drop_duplicates()
    polls = pd.merge(polls, invtable, how='outer', on='smoke_name')
    polls.rename(columns={'keep': 'in_invtable'}, inplace=True)
    polls['in_invtable'] = polls['in_invtable'].fillna('N')
    qa_out = os.path.join(work_path, 'qa', 'csvs', '%s_poll_qa.csv' %sector)
    polls.to_csv(qa_out, index=False)
    inv = inv[inv['keep'] == 'Y'].copy()
    ...
    count_out = os.path.join(work_path, 'qa', 'csvs', '%s_counts.csv' %sector)
    ...
    xwalk.drop_duplicates(['facility_id','unit_id','process_id','rel_point_id'], inplace=True)
    ...
    qa_out = os.path.join(work_path, 'qa', 'csvs', '%s_emis_qa.csv' %sector)
    ...
    if len(fug_params) > 0:
        ...
        qa_out = os.path.join(work_path, 'qa', 'csvs', '%s_fug_param_qa.csv' %sector)
        df.to_csv(qa_out, index=False)
    if len(point_params) > 0:
        df = check_point_params(point_params, inv)
        qa_out = os.path.join(work_path, 'qa', 'csvs', '%s_point_param_qa.csv' %sector)
        df.to_csv(qa_out, index=False)
    ...
    qa_out = os.path.join(work_path, 'qa', 'csvs', '%s_srcid_qa.csv' %sector)
