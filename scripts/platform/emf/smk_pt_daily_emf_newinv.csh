      ...
      if ( $HOUR_SPECIFIC_YN == Y ) then
	     ...
	     if ($NAMEBREAK_HOURLY == cmv) then
	       ...
	       setenv MULTIFILE_NAMEBREAK 5 # dummy numeric value
               ...
               set combine_style = ptegu
               setenv MULTFILE_NAMEBREAK 5
             else if ($NAMEBREAK_HOURLY == ptegu_ff10) then
               set combine_style = ptegu_ff10
               setenv MULTFILE_NAMEBREAK 5
	     else
         ...
         echo $prevmon
         if ($prevmon == 0) then
           set prev = "prevdec_2020"
         else
           set prev = "${prevmon}_2021"
         endif
         cat > $HOURLISTFILE << EOF
#LIST CEM
/work/EMIS/users/bte/WO250.1/cem2hour/ff10/pthour_${mon}_2021.csv
/work/EMIS/users/bte/WO250.1/cem2hour/ff10/pthour_$prev.csv
EOF
...
#      if ( $RUN_SMKREPORT == Y ) set anytmprep = Y
   ...
   while ( $n < $ndays )   # Expecting M_TYPE == all (for all days of year)
      ...
      if ($RUN_LAYPOINT == Y && $MET_ASM == Y) then
        ...
        ssh atmost cp -pv $MET_CRO_2D $IMD_ROOT/METCRO2D_${metdate}_$JOBSTAMP
        ssh atmost cp -pv $MET_CRO_3D $IMD_ROOT/METCRO3D_${metdate}_$JOBSTAMP
        ssh atmost cp -pv $MET_DOT_3D $IMD_ROOT/METDOT3D_${metdate}_$JOBSTAMP
