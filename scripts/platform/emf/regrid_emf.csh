...
if (! $?INGRID_PATH) then
   set inpath = $PREMERGED/..
   echo "SCRIPT NOTE: INGRID_PATH parameter is undefined; defaulting to current case."
else
...
set regrid       = $SCRIPTS/regrid/regrid.py
...
foreach mon ( $months )
	...
	foreach date (`tail -${days} ${smk_dat} | cut -d',' -f1`)
		...
		# set INFILE/OUTFILE differently if sector name is mrggrid, indicating we want to regrid model-ready files.
		# Also need to figure out BEISLABEL and RWCLABEL for this (defaults to 'withbeis' and 'withrwc' if SECTORLIST is undefined)
		if ($SECTOR == mrggrid) then 
                  ...
                  if ( $?SECTORLIST ) then
                    ...
                    if ( $isrwcmerged == Y ) then
                      setenv RWCLABEL withrwc
                    else 
                      setenv RWCLABEL norwc
                    endif
                  else
                    setenv RWCLABEL withrwc
                  endif
		  ...
		  setenv INFILE $OUTPUT/../../$INGRID/$EMF_SPC/emis_${MM}_all_${date}_${INGRID}_${BEISLABEL}_${RWCLABEL}_${CASE}.ncf
		  setenv OUTFILE $OUTPUT/emis_mole_all_${date}_${GRID}_${BEISLABEL}_${RWCLABEL}_${CASE}.ncf
