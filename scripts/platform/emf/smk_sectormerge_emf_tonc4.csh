...
## Initialize zip option to N, if not set
if ( ! $?COMPRESS_OUTPUTS_NC4 ) then
   setenv COMPRESS_OUTPUTS_NC4 N
endif
...
setenv REPMERGE_ADJ $REPOUT/mrggrid/$GRID/$SPC/rep_adj_{$namelabel}.csv
setenv REPMERGE_SUM $REPOUT/mrggrid/$GRID/$SPC/rep_adjsummary_{$namelabel}.csv
setenv REPMERGE_TAG $REPOUT/mrggrid/$GRID/$SPC/rep_tag_{$namelabel}.csv
...
foreach mon ( $MONTHS_LIST )    # Note that some months can appear more than once with different spin-up group
   ...
   setenv FILELIST $IMD_ROOT/merge/filelist_${CASE}_${GRID}_$mon.lst
...
#        # Also shorten all EVs to 16 characters or less because SMOKE can have problems otherwise
   ...
   #   Moved this farther up in the script on 17 Jun 2020 so that it could be used as part of readme
   setenv BEISLABEL $SPC # default
   ...
   if ( $?SECTORLIST ) then
     ...
     if ( $isbeismerged == Y || $isgbiogmerged == Y ) then 
       setenv BEISLABEL withbeis
     else 
       setenv BEISLABEL nobeis
     endif
     set isrwcmerged = `cut -f1,8 -d',' $SECTORLIST | grep '"rwc"' | cut -f2 -d',' | sed 's/\"//g'`
     if ( $isrwcmerged == Y ) then
       setenv BEISLABEL ${BEISLABEL}_withrwc
     else
       setenv BEISLABEL ${BEISLABEL}_norwc
     endif
   endif
   ...
   while ( $date_counter < $daysmonth )
      ...
      ## List of premerged files for documentation purposes
      #  An auto-generated README will be output to the model-ready directory which includes paths to each of the 
      #  premerged sectors and the location of the SECTORLIST
      #  Added by Christine Allen, 17 Jun 2020
      if (-e $OUTPUT/tmp_filelist_$$) rm $OUTPUT/tmp_filelist_$$
      if (-e /usr/bin/date) then
        set todaysdate = `/usr/bin/date | cut -c1-10`
	set todaylabel = "created on $todaysdate "
      else
        set todaylabel = ""
      endif	
      echo "The emis_mole_all $BEISLABEL files ${todaylabel}include the following premerged sectors:" > $OUTPUT/tmp_readme_$$
      echo "" >> $OUTPUT/tmp_readme_$$
	 ...
	 if ( $sector_mrgg[$seccnt] == N ) goto end_of_sectorloop
	    ...
	    echo $sector_file[$seccnt] >> $OUTPUT/tmp_filelist_$$
      	    echo $sector_file[$seccnt] >> $OUTPUT/tmp_readme_$$
	    ...
	    if (! -e $case_imd/mrggrid) mkdir -p $case_imd/mrggrid		   # 20 Feb 2023: automatically create premerged/mrggrid directory for older cases if it doesn't already exist
	    ...
	    set    sector_file[$seccnt] = $case_imd/mrggrid/emis_${MM}_${sector}_${esdate}_${GRID}_${speciation}_${case}.${EMF_JOBID}.tmp.ncf
            setenv $sector_ev[$seccnt] $sector_file[$seccnt]
       	    ...
       	    echo $case_imd/$sector/emis_${MM}_${sector}_${esdate}_${GRID}_${speciation}_${case}.ncf >> $OUTPUT/tmp_filelist_$$
       	    echo $case_imd/$sector >> $OUTPUT/tmp_readme_$$
      ...
      setenv OUTFILE    $OUTPUT/merged_${BEISLABEL}/emis_${MM}_all_${ESDATE}_${GRID}_${BEISLABEL}_${CASE}.ncf
      if ( ! -e $OUTPUT/merged_${BEISLABEL} ) mkdir -p $OUTPUT/merged_${BEISLABEL}
      setenv REPMRGGRID $REPOUT/$SECTOR/$GRID/$SPC/mrggrid_dates_$date.rpt
      if ( ! -e $REPOUT/$SECTOR/$GRID/$SPC ) then
         mkdir -p $REPOUT/$SECTOR/$GRID/$SPC
         chmod ug+rwx $REPOUT/$SECTOR/$GRID/$SPC
      endif

      echo "" >> $OUTPUT/tmp_readme_$$
      echo "Full sector list including representative date settings: $SECTORLIST" >> $OUTPUT/tmp_readme_$$
      mv -f $OUTPUT/tmp_readme_$$ $OUTPUT/merged_${BEISLABEL}/README_${CASE}_${GRID}_${BEISLABEL}_premerged_sectors.txt
	 ...
	 # Don't delete file if sectorcase == none, because then sector_file is the original file, not a softlink
      ...
      # Add list of premerged emissions as metadata in the form of DESCRIPTION attribute in merged emissions file
      # Added by Christine Allen 17 Jul 2020, as requested by Barron Henderson
      echo "TESTING here 1"
      ncatted -h -a "DESCRIPTION,global,a,c,Sum of files: " $OUTFILE
      set npremerged = `wc -l $OUTPUT/tmp_filelist_$$ | cut -f1 -d' '`
      set n = 0
      set alllines = ""
      while ($n < $npremerged)
        @ n++
	set thisline = `head -$n $OUTPUT/tmp_filelist_$$ | tail -1 `
	set alllines = "${alllines} ${thisline}"
      end
      ncatted -h -a "DESCRIPTION,global,a,c,$alllines" $OUTFILE
      rm -f $OUTPUT/tmp_filelist_$$
      echo "TESTING here 2"
	 ...
	 echo "Running domaintotals for $date"
         $domain_totals $REPOUT/$SECTOR/$GRID/$SPC/domain_totals_$date.rpt -f $OUTFILE -a -S $SPC -c NCF -o CSV domain
         if ( $status != 0 ) then
      ...
      #  GZIP takes precedence over NC4 compression
      if ( $GZIP_OUTPUTS == Y ) then
         /bin/gzip -f $OUTFILE
      else if ( $COMPRESS_OUTPUTS_NC4 == Y ) then
         setenv OUTFILE_NC4    $OUTPUT/merged_${BEISLABEL}/emis_${MM}_all_${ESDATE}_${GRID}_${BEISLABEL}_${CASE}.nc4
         nccopy -d1 $OUTFILE $OUTFILE_NC4
	 if ($status == 0 && -e $OUTFILE_NC4) rm -f $OUTFILE
      endif
...
set mrgcnt = `/bin/ls -1 $OUTPUT/merged_${BEISLABEL}/emis_${MM}_all_*_${GRID}*.$fileext | wc -l`
