...
# 1) Sol batch nodes cannot aput, so instead ssh to interactive node sol2 for
#    all aput commands
...
#
# 23 Aug 2017: Changes for atmos:
# - Changed how the group name is extracted from EMF_QUEUE_OPTIONS
# - Included error message indicating that job should be run on singlepe queue
# - removed "ssh sol2" from aput commands since singlepe has access to asm
# - removed some old code relating to terra and garnet
...
cp /asm/ROMO/README ~/asm_readme_temp
if ( $status != 0 ) then
   echo "SCRIPT ERROR: Submit this job to the singlepe queue, not the compute queues, so that the job has access to ASM."
   exit (1)
endif
...
while ( $nl < $nlines )
   ...
   if ( $nl > 1 ) then   # skip header line
	 ...
	 if ($status == 0) then # alternate ASM directory found
	    set new_asm_root = `grep ^$asm_root2/$projectx ~/asm_readme_temp | sort | head -1 | cut -f2 -d'>' | sed 's/ //g'`
...
#	    endif
	    ...
	    if ($ASM_TEMPLATE != "") then
	       ...
	       #   apportionment are archived
  	      aput -q -a $asm_dir1/$sector_name $asm_temp $sector_project/$sector_case/premerged/$sector_name/emis_mole_${sector_name}_????${mon}??_${GRID}_${sector_spc}_${sector_case}*ncf*
	      ...
	      if ($t1 > 0) then
	        ...
	        gzip -v $sector_project/$sector_case/premerged/*_${sector_name}_????${mon}??_${GRID}_*${sector_case}*ncf
  	        aput -q -d -a $asm_dir2/$sector_name $asm_temp $sector_project/$sector_case/premerged/*_${sector_name}_????${mon}??_${GRID}_*${sector_case}*ncf.gz
	      else
                aput -q -a $asm_dir2/$sector_name $asm_temp $out_rootx/$projectx/smoke_out/$sector_case/$GRID/$sector_spc/$sector_name/*_${sector_name}_????${mon}??_${GRID}_*${sector_case}*ncf.gz
	      endif
		  
            end # foreach
	    
	    # aput non-date-specific STACK_GROUPS from original location
            aput -q -a $asm_dir2/$sector_name $asm_temp $OUT_ROOT/$projectx/smoke_out/$sector_case/$GRID/$sector_spc/$sector_name/stack_groups_${sector_name}_${GRID}_${sector_case}.ncf*
	    ...
	    echo "archiving $sector_project/$sector_case/reports/smkmerge/${sector_name}_${GRID}.tar"
            aput -q -d -a $asm_dir_rep/smkmerge $asm_temp $sector_project/$sector_case/reports/smkmerge/${sector_name}_${GRID}.tar 
...
if ($status == 0) then # alternate ASM directory found
   set new_asm_root = `grep ^$asm_root2/$PROJECT ~/asm_readme_temp | sort | head -1 | cut -f2 -d'>' | sed 's/ //g'`
...
# if any unzipped files: copy all, gzip copies, aput -d. if no unzipped files, aput directly
set t1 = `ls -1 $OUTPUT/emis_mole_all_*_${GRID}_*_${CASE}*ncf $OUTPUT/merged*/emis_mole_all_*_${GRID}_*_${CASE}*ncf | wc -l`
if ($t1 > 0) then
  cp -pv $OUTPUT/emis_mole_all_*_${GRID}_*_${CASE}*ncf* $OUTPUT/merged*/emis_mole_all_*_${GRID}_*_${CASE}*ncf* $OUTPUT/stack_groups_* $IMD_ROOT
  gzip -v $IMD_ROOT/emis_mole_all_*_${GRID}_*_${CASE}*ncf
  aput -q -d -a $asm_dir2 $asm_temp $imd_rootx/emis_mole_all_*_${GRID}_*_${CASE}*ncf.gz
  aput -q -d -a $asm_dir2 $asm_temp $imd_rootx/stack_groups_*
else
  aput -q -a $asm_dir2 $asm_temp $OUTPUT/emis_mole_all_*_${GRID}_*_${CASE}*ncf.gz $OUTPUT/merged*/emis_mole_all_*_${GRID}_*_${CASE}*ncf.gz
  aput -q -a $asm_dir2 $asm_temp $OUTPUT/stack_groups_*
endif
...
#      aput -q -d -a $asm_dir2 $asm_temp $IMD_ROOT/stack_groups_*_${GRID}_*.ncf*
#   else
#      aput -q -a $asm_dir2 $asm_temp $OUTPUT/stack_groups_*_${GRID}_*.ncf*
#   endif	       
#endif
...
aput -q -d -a $asm_root/em_${PLATFORM}/$PROJECT/$CASE/reports/inv $repoutx/inv/*.tar
popd
