...
c17 ifort -fPIC -check -traceback -extend_source -zero -O3 -mp1 -o apply_precip_adj.int17 apply_precip_adj.f -L/home/local-rhel7/apps/netcdf-4.4.1/intel-17.0/lib -lnetcdf -lnetcdff -L/home/local-rhel7/apps/ioapi-3.2/intel-17.0/lib -lioapi -module /home/local-rhel7/apps/ioapi-3.2/intel-17.0/Linux2_x86_64ifort
...
C          include 'IODECL3.EXT'     ! I/O API function declarations 
       ...
       real, allocatable :: rn(:,:), rc(:,:), snow_cover(:,:)
       real, allocatable :: soim1(:,:), soit1(:,:), wspd10(:,:), sltyp(:,:)
...
C        external envreal
	...
	TSTEP3D = tstep
     ...
     &           infile( 1: TRIMLEN( infile ) ) 
	   ...
	   enddo
	   if ( .not. 
     &        read3(metcro2d,'RN',1,jdate,jtime,rn)
     &        ) then
              mesg = 'Error reading from file '//
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
              call m3exit( progname, 0, 0, MESG, 2 )
	   ...
	   if ( .not. 
     &        read3(metcro2d,'RC',1,jdate,jtime,rc)
     &        ) then
              mesg = 'Error reading from file '//
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
              call m3exit( progname, 0, 0, MESG, 2 )
     ...
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
	   ...
	   if ( .not. 
     &        read3(metcro2d,'SOIT1',1,jdate,jtime,soit1)
     &        ) then
              mesg = 'Error reading from file '//
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
              call m3exit( progname, 0, 0, MESG, 2 )
     ...
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
	   ...
	   if ( .not. 
     &        read3(metcro2d,'WSPD10',1,jdate,jtime,wspd10)
     &        ) then
              mesg = 'Error reading from file '//
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
              call m3exit( progname, 0, 0, MESG, 2 )
...
!                   if ((rn(c_loop, r_loop) +rc(c_loop, r_loop)) .ge. 0.0) then
...
!                   if (wspd10(c_loop,r_loop) .eq. 0.0) then
!                      buffer(c_loop,r_loop,l_loop,v_loop) = 
!    &            zero*buffer(c_loop,r_loop,l_loop,v_loop)
!		   endif
	   ...
	   enddo
	     ...
	     do l_loop = 1, nlays
		   ...
		   do c_loop = 1, ncols
			  ...
			  if ((ratio .lt. 0.0) .or. (ratio .gt. 1.0)) then
			     write (logdev,*) 'bad ratio at ', c_loop, r_loop,soim1(c_loop,r_loop), saturation(sltyp(c_loop,r_loop)) 
			     stop
			  endif
