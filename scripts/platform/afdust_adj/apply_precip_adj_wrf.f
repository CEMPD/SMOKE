...
C          include 'IODECL3.EXT'     ! I/O API function declarations 
       ...
       real, allocatable :: soim1(:,:), soit1(:,:), wspd10(:,:), sltyp(:,:), lai(:,:)
...
C        external envreal
	...
	TSTEP3D = tstep
     ...
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
	   ...
	   if ( .not. 
     &        read3(metcro2d,'LAI',1,jdate,jtime,lai)
     &        ) then
              mesg = 'Error reading from file '//
     &           metcro2d( 1: TRIMLEN( metcro2d ) ) 
              call m3exit( progname, 0, 0, MESG, 2 )
...
!			        write (logdev,*) 'bad ratio at ',
!     &    c_loop, r_loop,sltyp(c_loop,r_loop),soim1(c_loop,r_loop), 
!     &    saturation(sltyp(c_loop,r_loop)) 
!			        stop
!			     endif
