...
C.........  Read names of input files and open files
        MESG = 'Enter logical name for a list input file for stack ' //
     &         'group input files'
        IDEV = PROMPTFFILE( MESG, .TRUE., .TRUE.,
     &                      'FILELIST_STACK', PROGNAME   )

        MESG = 'Enter logical name for a list input file for inline-' //
     &         'ready emissions input files'
        JDEV = PROMPTFFILE( MESG, .TRUE., .TRUE.,
     &                      'FILELIST_INLN', PROGNAME   )
     
C.........  Determine maximum number of input files in file
        MXNFIL_1 = GETFLINE( IDEV, '' )

        MXNFIL_2 = GETFLINE( JDEV, '' )
...
C...........................  Search for adj factor species in the logical file
                            IF( ADEV .GT. 0 ) THEN
                                ...
                                IF( LFNTMP == NAM ) THEN
                                    ...
                                    K = INDEX1( SPCTMP, NVARSA(F,S),
     &                                  VNAMEA(1,F,S) )

                                    IF( K <= 0 ) THEN
                                       EFLAG = .TRUE.
                                       MESG = 'WARNING: The species '//
     &                                     TRIM( ADJ_SPC(J) ) // 
...
C..........................  If its not in the output list, add it
     ...
     &                                          VAR_I(1,1:NSRC(S,F),1)
                            STACK_PARAM_I_OUT(ISTART:IEND,NVAR_INT)  = 
     &                                          VAR_I(1,1:NSRC(S,F),1)
                        ELSE
