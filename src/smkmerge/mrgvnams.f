...
C   begin body of subroutine MRGVNAMS
        ...
        IF( ALLOCATED( INVDCOD ) ) DEALLOCATE( INVDCOD )
        IF( ALLOCATED( INVDNAM ) ) DEALLOCATE( INVDNAM )
        IF( ALLOCATED( INVSTAT ) ) DEALLOCATE( INVSTAT )
        IF( ALLOCATED( INVDUNT ) ) DEALLOCATE( INVDUNT )
        IF( ALLOCATED( INVDCNV ) ) DEALLOCATE( INVDCNV )

        IF( ALLOCATED( EANAM   ) ) DEALLOCATE( EANAM    )
        IF( ALLOCATED( EINAM   ) ) DEALLOCATE( EINAM    )
        IF( ALLOCATED( TONAMES ) ) DEALLOCATE( TONAMES  )
        IF( ALLOCATED( TOUNITS ) ) DEALLOCATE( TOUNITS  )
        IF( ALLOCATED( SPCUNIT ) ) DEALLOCATE( SPCUNIT  )
        IF( ALLOCATED( INDXA   ) ) DEALLOCATE( INDXA    )
        IF( ALLOCATED( TVSORTA ) ) DEALLOCATE( TVSORTA  )
        IF( ALLOCATED( TVDESCA ) ) DEALLOCATE( TVDESCA  )
        IF( ALLOCATED( TSVDESC ) ) DEALLOCATE( TSVDESC  )
        IF( ALLOCATED( AEMNAM  ) ) DEALLOCATE( AEMNAM   )
        IF( ALLOCATED( BEMNAM  ) ) DEALLOCATE( BEMNAM   )
        IF( ALLOCATED( MEMNAM  ) ) DEALLOCATE( MEMNAM   )
        IF( ALLOCATED( PEMNAM  ) ) DEALLOCATE( PEMNAM   )
        IF( ALLOCATED( EMNAM   ) ) DEALLOCATE( EMNAM    )
        IF( ALLOCATED( EMIDX   ) ) DEALLOCATE( EMIDX    )
        IF( ALLOCATED( TMPBNAM ) ) DEALLOCATE( TMPBNAM  )
        IF( ALLOCATED( TMPBIDXA) ) DEALLOCATE( TMPBIDXA )
        IF( ALLOCATED( TMPBIDXB) ) DEALLOCATE( TMPBIDXB )
